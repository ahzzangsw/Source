...
public class AddShot : Projectile
{
    private Transform m_TargetTransform = null;

    void Awake()
    {
        eProjectileType = ProjectileType.ADDSHOT;
    }
    ...
    private IEnumerator AttackDelay()
    {
        if (m_Master)
        {
            for (int i = 1; i < m_Master.TargetCount; ++i)
            {
                yield return new WaitForSeconds(0.2f); // 0.1ÃÊ µô·¹ÀÌ
                Fire(m_TargetTransform, false);
            }
        }
    }
